//Viết chương trình đọc vào một đơn đồ thị có hướng không chu trình, 
//xếp hạng các đỉnh và in hạng của các đỉnh ra màn hình.
#include <stdio.h>
#define MAX_N 100
...
void topo_rank (Graph *pG){
    ...
    int k = 0; //hạng tính từ 0

    //loop chính, lặp đến khi s1 rỗng thì dừng
    while (S1.size > 0){
        init_list(&S2);
        for (int i = 0; i < S1.size; i++){
            int u = element_at(&S1,i); //lấy hạng gốc u trong S1 
            r[u] = k; // xếp hạng cho u

            //xóa đỉnh u <=> giảm bậc vào các đỉnh kề v của u
            for (int v = 1; v <= pG->n; v++){
                if (pG->A[u][v] != 0){
                    d[v]--;
                    if (d[v] == 0){
